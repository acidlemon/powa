...
const authSessionName = "powa_auth_session"
const commonSessionName = "powa_session"
const gateSessionName = "powa_gate"
...
func (s *Server) init() {
	...
	<form method="get" action="{{ path "/start" }}">
	...
	funcMap := template.FuncMap{
		"path": func(p string) string {
			return path.Join(s.mountOn, p)
		},
	}

	layout := template.Must(template.New("layout").Parse(layoutHTML)).Funcs(funcMap)
	signInPage = template.Must(template.Must(layout.Clone()).Parse(signInTmpl))
}

func (s *Server) startAuthHandler(w http.ResponseWriter, req *http.Request) {
	// return path
...
func (s *Server) callbackAuthHandler(w http.ResponseWriter, req *http.Request) {
...
<body>
<main>
  <form method="post" action="` + path.Join(s.mountOn, "/verify") + `">
...
func (s *Server) verifyAuthHandler(w http.ResponseWriter, req *http.Request) {
	...
	returnPath := session.Values["return_path"].(string)
	if returnPath == "" {
		returnPath = "/"
	}
...
func (s *Server) signInHandler(w http.ResponseWriter, req *http.Request) {
	gateSession, err := s.store.Get(req, gateSessionName)
	if err != nil {
		log.Println("failed to get gate session:", err.Error())
	}

	pathVal, _ := gateSession.Values["path"].(string)
...
func (s *Server) unauthorizedHandler(w http.ResponseWriter, req *http.Request) {
...
<body>
<header>
  <h1 style="border-bottom: 3px double ">{{ .Title }}</h1>
  <h2>{{ .Heading }}</h2>
</header>
{{ template "main" . }}
</body>
